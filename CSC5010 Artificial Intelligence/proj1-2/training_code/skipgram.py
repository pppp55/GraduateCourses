...
local_vec = Path(__file__).with_name("skipgram.vec")
target_vec = Path(__file__).resolve().parents[1] / "embeddings" / "skipgram.vec"
save_pretrained(vocab, model.embeddings.weight.data, local_vec)
try:
    shutil.copyfile(local_vec, target_vec)
    print(f"Copied pretrained embeddings to: {target_vec}")
except PermissionError as exc:
    print(f"Could not copy to {target_vec}: {exc}")
    print(f"Please copy {local_vec} to {target_vec} manually.")
